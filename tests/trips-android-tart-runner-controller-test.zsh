...
case "${FAKE_SCENARIO:-}" in
  tonegate-only)
    if [[ "$request" == *'repos/jai/tonegate/actions/runs?'* ]]; then
      ...
      print -r -- '[{"jobs":[{"status":"queued","created_at":"2026-08-25T00:00:01Z","labels":["self-hosted","macOS","ARM64","tart","android"]}]}]'
    fi
    ;;
  trips-first)
    if [[ "$request" == *'repos/jai/trips-frontend/actions/runs?'* ]]; then
      ...
      print -r -- '[{"jobs":[{"status":"queued","created_at":"2026-08-25T00:00:02Z","labels":["self-hosted","macOS","ARM64","tart","android","borg-cube-03"]}]}]'
    fi
    ;;
  incompatible-host)
    if [[ "$request" == *'repos/jai/trips-frontend/actions/runs?'* ]]; then
      ...
      print -r -- '[{"jobs":[{"status":"queued","created_at":"2026-08-25T00:00:03Z","labels":["self-hosted","macOS","ARM64","tart","android","another-host"]}]}]'
    fi
    ;;
  no-jobs)
    if [[ "$request" == *'repos/jai/trips-frontend/actions/runs?'* ]]; then
...
vms=[{"Name":"trips-android-runner-base","Running":False,"State":"stopped","Source":"local"}]
...
if state:
    vms.append({"Name":"trips-android-runner-job-stale","Running":state == "running","State":state,"Source":"local"})
print(json.dumps(vms))'
elif [[ "$1" == list && "${FAKE_VM_PRESENT:-false}" == true ]]; then
  print -r -- 'test-vm'
elif [[ "$1" == list && "${FAKE_STALE_VM_PRESENT:-false}" == true ]]; then
  print -r -- 'trips-android-runner-job-stale'
fi
exit 0
SCRIPT
chmod 700 "$fake_tart"
...
export TRIPS_ANDROID_TART_REPOSITORIES='jai/trips-frontend,jai/tonegate'
export TRIPS_TART_CLAIM_TIMEOUT_SECONDS=1
export TRIPS_TART_CLAIM_POLL_SECONDS=0.1
export TRIPS_ANDROID_TART_CONTROLLER_LOCK="${test_directory}/controller.lock"
export TRIPS_TART_NATIVE_LANE_LOCK="${test_directory}/native-lane.lock"
source "${repo_root}/scripts/trips-android-tart-runner-controller.zsh"
...
assert_equal 5 "$minimum_root_free_gib"
assert_equal 'borg-cube-03,tart,android' "$runner_labels"
android_guest_preflight_command=$(functions android_guest_preflight)
[[ "$android_guest_preflight_command" == *'/Volumes/RunnerWork/android-sdk'* ]]
[[ "$android_guest_preflight_command" == *'ci-android-arm64'* ]]
[[ "$android_guest_preflight_command" == *'-accel-check'* ]]
...
assert_equal 'trips-android-runner-job-stale' "$(list_ephemeral_vms)"
...
if /usr/bin/grep -Eq '^(stop|delete) trips-android-runner-job-stale$' "$FAKE_TART_RUN_LOG"; then
...
/usr/bin/grep -q '^delete trips-android-runner-job-stale$' "$FAKE_TART_RUN_LOG"
...
if TRIPS_RUNNER_CONTROLLER_LIBRARY_ONLY=true \
  TRIPS_ANDROID_TART_CONTROLLER_LOCK="$TRIPS_ANDROID_TART_CONTROLLER_LOCK" \
  zsh -uc 'source "$1"; acquire_controller_lock' \
  _ "${repo_root}/scripts/trips-android-tart-runner-controller.zsh"; then
...
TRIPS_RUNNER_CONTROLLER_LIBRARY_ONLY=true \
  ...
  _ "${repo_root}/scripts/trips-android-tart-runner-controller.zsh" "${test_directory}/first.raw" "${test_directory}/first.log" "$lane_ready" "$lane_release" &
first_contender_pid=$!
for _ in {1..50}; do
  [[ -e "$lane_ready" ]] && break
  sleep 0.02
...
TRIPS_RUNNER_CONTROLLER_LIBRARY_ONLY=true \
  ...
  _ "${repo_root}/scripts/trips-android-tart-runner-controller.zsh" "${test_directory}/second.raw" "${test_directory}/second.log"
: > "$lane_release"
...
assert_configured_network_invocation() {
  ...
  if [[ "$configured_mode" == unset ]]; then
    env -u TRIPS_TART_NETWORK_MODE \
      ...
      _ "${repo_root}/scripts/trips-android-tart-runner-controller.zsh" "$vm_name" "$work_disk" "$vm_log"
  else
