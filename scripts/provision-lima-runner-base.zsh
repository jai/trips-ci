...
/opt/homebrew/bin/limactl shell "$base_vm" -- \
  bash -lc 'set -e; test "$(uname -m)" = aarch64; test "$(nproc)" = 3; test "$(free -g | awk '\''/^Mem:/{print $2}'\'')" -ge 7; sudo docker info >/dev/null; docker compose version; docker buildx version; node --version; npm --version; test -x /opt/actions-runner/bin/Runner.Listener'
